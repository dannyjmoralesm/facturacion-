...
export function createWhatsAppSaleMessage(sale: Sale, profile: BusinessProfile): string {
  ...
  lines.push(`🧾 *${(profile?.commercialName || profile?.name || 'NEGOFACT').toUpperCase()}*`);
  ...
  lines.push(`_NegoFact POS Bimoneda Venezuela_`);
...
export function createWhatsAppQuoteMessage(quote: Quote, profile: BusinessProfile): string {
  ...
  lines.push(`🏢 *${profile?.commercialName || profile?.name || 'NEGOFACT'}* (RIF: ${profile?.rif || 'J-00000000-0'})`);
  lines.push(`--------------------------------`);
...
export function createWhatsAppCatalogMessage(products: Product[], bcvRate: number, profile: BusinessProfile): string {
  ...
  lines.push(`🏢 *${profile?.commercialName || profile?.name || 'NEGOFACT'}*`);
...
export function createWhatsAppSupplierPaymentMessage(
  ...
  lines.push(`🏢 *${profile?.commercialName || profile?.name || 'NEGOFACT'}* (RIF: ${profile?.rif || ''})`);
  lines.push(`--------------------------------`);
